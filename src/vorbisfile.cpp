...
int64_t ov_raw_total(OggVorbis_File *vf, int i) {
	...
	if(i < 0) {
		int64_t acc = 0;
		int     i;
		for(i = 0; i < vf->links; i++) acc += ov_raw_total(vf, i);
		return acc;
	}
...
int64_t ov_pcm_total(OggVorbis_File *vf, int i) {
	...
	if(i < 0) {
		int64_t acc = 0;
		int     i;
		for(i = 0; i < vf->links; i++) acc += ov_pcm_total(vf, i);
		return acc;
	}
...
int64_t ov_time_total(OggVorbis_File *vf, int i) {
	...
	if(i < 0) {
		int64_t acc = 0;
		int     i;
		for(i = 0; i < vf->links; i++) acc += ov_time_total(vf, i);
		return acc;
	}
	else { return ((int64_t)vf->pcmlengths[i * 2 + 1]) * 1000 / vf->vi.rate; }
}
				...
				if(result > 0) {
					if(vf->vi.codec_setup) {
						...
						if(thisblock < 0) {
							...
							if(eosflag) ogg_stream_packetout(vf->os, NULL);
							else if(lastblock)
...
int ov_pcm_seek(OggVorbis_File *vf, int64_t pos) {
	...
	while(1) {
		...
		if(ret > 0) {
			...
			if(op.granulepos > -1) {
				...
				for(i = 0; i < vf->current_link; i++) vf->pcm_offset += vf->pcmlengths[i * 2 + 1];
			}
	...
	while(vf->pcm_offset < pos) {
		...
		int32_t samples = vorbis_dsp_pcmout(vf->vd, NULL, 0);
		...
		if(samples < target)
			if(_fetch_and_process_packet(vf, 1, 1) <= 0) vf->pcm_offset = ov_pcm_total(vf, -1); /* eof */
	}
