...
int _bisect_forward_serialno(OggVorbis_File *vf, int64_t begin,
		int64_t searched, int64_t end, uint32_t currentno, long m) {
...
int _fetch_headers(OggVorbis_File *vf, vorbis_info *vi, vorbis_comment *vc,
		uint32_t *serialno, ogg_page *og_ptr) {
	ogg_page og = { 0, 0, 0, 0 };
...
int _ov_open1(void *f, OggVorbis_File *vf, char *initial, long ibytes,
...
long ogg_sync_pageseek(ogg_sync_state_t *oy, ogg_page *og) {
	 ...
	 checksum */
	{
		...
		if (chksum
			...
			/* D'oh.  Mismatch! Corrupt page (_or miscapture and not a page
	...
	while (oy->fifo_tail) {
		...
		unsigned char *next = (uint8_t*)memchr(now, 'O', oy->fifo_tail->length);
...
uint32_t _checksum(ogg_reference_t *_or, int bytes) {
	...
	while (_or) {
		unsigned char *data = _or->buffer->data + _or->begin;
...
ogg_reference_t* ogg_buffer_split(ogg_reference_t **tail, ogg_reference_t **head, long pos) {
	...
	if (!_or || pos == 0) {
		...
		if (pos >= _or->length) {
			...
			long lengthA = pos;
			long beginB = _or->begin + pos;
			long lengthB = _or->length - pos;

			/* make a new reference to tail the second piece */
			*tail = _fetch_ref(_or->buffer->ptr.owner);
...
ogg_reference_t* ogg_buffer_dup(ogg_reference_t *_or) {
	...
	while (_or) {
		ogg_reference_t *temp = _fetch_ref(_or->buffer->ptr.owner);
		if (head)
			head->next = temp;
		else
			ret = temp;
