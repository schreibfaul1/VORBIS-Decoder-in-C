...
static int _make_decode_table(codebook *s, char *lengthlist, long quantvals,
	...
	if (s->dec_nodeb == 4) {
		...
		if (_make_words(lengthlist, s->entries, s->dec_table, quantvals, s, opb,
				maptype))
			return 1;
...
long vorbis_book_decodevs_add(codebook *book, int32_t *a, oggpack_buffer *b,
		int n, int point) {
...
/* decode vector / dim granularity guarding is done in the upper layer */
long vorbis_book_decodev_add(codebook *book, int32_t *a, oggpack_buffer *b,
		int n, int point) {
	if (book->used_entries > 0) {
		int32_t *v = (int32_t*) alloca(sizeof(*v) * book->dim);
		int i, j;
 ...
 * floor0) */
long vorbis_book_decodev_set(codebook *book, int32_t *a, oggpack_buffer *b,
		int n, int point) {
	if (book->used_entries > 0) {
		int32_t *v = (int32_t*) alloca(sizeof(*v) * book->dim);
		int i, j;
...
/* decode vector / dim granularity guarding is done in the upper layer */
long vorbis_book_decodevv_add(codebook *book, int32_t **a, long offset, int ch,
