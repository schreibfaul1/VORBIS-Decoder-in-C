...
void vorbis_info_clear(vorbis_info *vi){
  ...
  if(ci){
    ...
    if(ci->mode_param)free(ci->mode_param);
...
static int _vorbis_unpack_books(vorbis_info *vi,oggpack_buffer *opb){
  ...
  ci->floor_param=malloc(sizeof(*ci->floor_param)*ci->floors);
  ci->floor_type=malloc(sizeof(*ci->floor_type)*ci->floors);
  ...
  ci->residue_param=malloc(sizeof(*ci->residue_param)*ci->residues);
  for(i=0;i<ci->residues;i++)
    if(res_unpack(ci->residue_param+i,vi,opb))goto err_out;

  /* map backend settings */
  ci->maps=oggpack_read(opb,6)+1;
  ci->map_param=malloc(sizeof(*ci->map_param)*ci->maps);
  for(i=0;i<ci->maps;i++){
    if(oggpack_read(opb,16)!=0)goto err_out;
  ...
  ci->mode_param=
    (vorbis_info_mode *)malloc(ci->modes*sizeof(*ci->mode_param));
