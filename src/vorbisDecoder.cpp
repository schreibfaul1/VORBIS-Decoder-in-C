...
int _vorbis_unpack_comment(vorbis_comment *vc, oggpack_buffer *opb) {
	...
	if (vc->comments < 0)
		goto err_out;
	vc->user_comments = (char**) calloc(vc->comments + 1,
			sizeof(*vc->user_comments));
	vc->comment_lengths = (int*) calloc(vc->comments + 1,
	...
	return (0);
	err_out: vorbis_comment_clear(vc);
	return (OV_EBADHEADER);
}
//-------------------------------------------------------------------------------------------------
...
int _vorbis_unpack_books(vorbis_info *vi, oggpack_buffer *opb) {
	...
	for (i = 0; i < ci->books; i++)
		if (vorbis_book_unpack(opb, ci->book_param + i))
			goto err_out;
	...
	for (i = 0; i < ci->residues; i++)
		if (res_unpack(ci->residue_param + i, vi, opb))
			goto err_out;
	...
	for (i = 0; i < ci->maps; i++) {
		if (oggpack_read(opb, 16) != 0)
			goto err_out;
		if (mapping_info_unpack(ci->map_param + i, vi, opb))
			goto err_out;
	}

	/* mode settings */
	ci->modes = oggpack_read(opb, 6) + 1;
	ci->mode_param = (vorbis_info_mode*) malloc(
			ci->modes * sizeof(*ci->mode_param));
...
int mapping_info_unpack(vorbis_info_mapping *info, vorbis_info *vi,	oggpack_buffer *opb) {
	...
	err_out: mapping_clear_info(info);
	return -1;
}
//-------------------------------------------------------------------------------------------------
int mapping_inverse(vorbis_dsp_state *vd, vorbis_info_mapping *info) {
	...
	int32_t **floormemo = (int32_t**)alloca(sizeof(*floormemo) * vi->channels);
	...
	for (i = 0; i < vi->channels; i++) {
		...
		if (ci->floor_type[floorno]) {
			/* floor 1 */
			floormemo[i] = (int32_t*)alloca(sizeof(*floormemo[i])	* floor1_memosize(ci->floor_param[floorno]));
			floormemo[i] = floor1_inverse1(vd, ci->floor_param[floorno],
					floormemo[i]);
		} else {
			/* floor 0 */
			floormemo[i] = (int32_t*)alloca(sizeof(*floormemo[i])	* floor0_memosize(ci->floor_param[floorno]));
			floormemo[i] = floor0_inverse1(vd, ci->floor_param[floorno],
					floormemo[i]);
		}
...
//-------------------------------------------------------------------------------------------------
int bitrev12(int x) {
	unsigned char bitrev[16] = { 0, 8, 4, 12, 2, 10, 6, 14, 1, 9, 5, 13, 3, 11,
			7, 15 };
